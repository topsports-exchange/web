...
import PlaceBet, { BetInterface, PlaceBetPopup } from "./PlaceBet";
  ...
  return (
    <>
      <div className="w-96  bg-neutral-950 rounded-2xl flex flex-col justify-center items-center p-4 fixed top-20 right-0">
        {isLogged ? <MyBetsLoggedIn /> : <MyBetsNotLogged />}
      </div>
      <PlaceBetPopup betData={betDataMock} />
      {/* <PlaceBetPopup isOpen={isLogged} onClose={() => setIsLogged(false)} /> */}
    </>
  );
};
export const MyBetsLoggedIn = () => {
  return (
    <div className="">
      ...
      <Tabs />
      {/* <PlaceBet betData={betDataMock} /> */}
      <TabWins bets={[betMock, { ...betMock, actionText: undefined }]} />{" "}
    </div>
  );
};
...
const Tabs = () => {
  return (
    <div className=" bg-[#0d2521ec] rounded-full flex text-white">
      <button className="bg-emerald-400 px-4 py-1 rounded-full font-semibold transition duration-300 ease-in-out focus:outline-none">
        Pending
      </button>
      <button className="px-4 py-1 font-semibold transition duration-300 ease-in-out focus:outline-none">Wins</button>
      <button className="px-4 py-1 font-semibold transition duration-300 ease-in-out focus:outline-none">
        History
      </button>
    </div>
  );
...
const TabPending = (args: { bets: Bet[] }) => {
  return (
    <>
      {args.bets.map((bet, index) => (
        <TabBet key={index} bet={bet} />
...
const TabBet = (args: { bet: Bet }) => {
  const { bet } = args;
  return (
    <div className="bg-neutral-950 rounded-2xl p-4 text-white">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          <img src={bet.teamLogo} alt={bet.teamName} className="w-6 h-6 mr-2" />
          <span>{bet.teamName}</span>
        </div>
        <div className="text-emerald-400">{bet.odds}</div>
      </div>
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center">
          <img src={bet.opponentLogo} alt={bet.opponentName} className="w-6 h-6 mr-2" />
          <span>{bet.opponentName}</span>
        </div>
      </div>
      <div className="mb-4">
        <div>
          {bet.teamName} Wins @ {bet.odds}
        </div>
        ...
        <div className="flex justify-between">
          ...
          <span className="text-emerald-400">{bet.potentialWinnings}</span>
        </div>
      </div>
      {bet.actionText && (
